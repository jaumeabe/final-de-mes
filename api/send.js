...
const LABELS = {
        num_machos:               'Numero de machos',
        num_cerdas:               'Numero de cerdas',
        num_primerizas:           'Numero de primerizas',
        lechones_maternidad:      'Lechones maternidad',
        lechones_destete:         'Lechones destete',
        primerizas_entradas:      'N primerizas entradas',
        acumulado_primerizas:     'Acumulado primerizas',
        machos_entrados:          'N machos entrados',
...
function formatMes(mesRaw) {
        if (!mesRaw) return mesRaw;
        ...
        return mesRaw;
                                            ...
                                            function buildHtml(data, mesLabel) {
                                                    const sectionRows = SECTIONS.map(function(sec) {
                                                              const rows = sec.fields.map(function(f) {
                                                                          return '<tr><td style="padding:6px 12px;color:#4a6080;font-size:13px;">' + LABELS[f] + '</td><td style="padding:6px 12px;text-align:right;font-weight:600;color:#1a3a5c;font-size:13px;">' + (data[f] || '—') + '</td></tr>';
                                                              }).join('');
                                                              return '<tr><td colspan="2" style="background:#1a3a5c;color:#fff;font-size:11px;font-weight:700;letter-spacing:0.06em;padding:7px 12px;text-transform:uppercase;">' + sec.title + '</td></tr>' + rows;
                                                    }).join('');

  return '<div style="font-family:Segoe UI,Arial,sans-serif;max-width:580px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #dde3ec;">' +
            '<div style="background:#1a3a5c;padding:24px 32px;">' +
            '<h1 style="margin:0;color:#fff;font-size:20px;letter-spacing:0.04em;text-transform:uppercase;">Inventario — Granja</h1>' +
            '<p style="margin:6px 0 0;color:#a8c4e0;font-size:14px;">' + data.granja + ' | ' + mesLabel + '</p>' +
            '</div>' +
            '<div style="padding:8px 20px 24px;"><table style="width:100%;border-collapse:collapse;">' + sectionRows + '</table></div>' +
            '<div style="background:#f0f2f5;padding:12px 24px;font-size:11px;color:#7a8fa8;text-align:center;">Enviado automaticamente desde el formulario Final de Mes - Premier Pigs</div>' +
            '</div>';
                                            }
...
export default async function handler(req, res) {
  ...
  const mesLabel = formatMes(data.mes);

  try {
            const result = await resend.emails.send({
                        from:    'Final de Mes <onboarding@resend.dev>',
                        to:      DESTINATARIOS,
                        subject: 'Inventario ' + data.granja + ' — ' + mesLabel,
                        html:    buildHtml(data, mesLabel),
            });

          return res.status(200).json({ ok: true, id: result.data && result.data.id });
  } catch (err) {
            console.error('Error enviando email:', err);
            return res.status(500).json({ error: 'Error al enviar el email', detail: String(err) });
  }
}
