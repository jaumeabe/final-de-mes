...
export default async function handler(req, res) {
  ...
  try {
      ...
      // Guardar en KV para el resumen mensual
      const mesKey = `inventario:${data.mes}`;
        const existing = await kv.get(mesKey) || {};
        existing[data.granja] = { ...data, timestamp: new Date().toISOString() };
        await kv.set(mesKey, existing);
